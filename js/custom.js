...
---------------------------------------------------------------------------------------------*/
	...
	function Submenu_toogle_adminnav() {
		...
		jQuery(
			"<div class='fa fa-angle-right open-close-admin-btn'></div>"
		).insertAfter(".admin-nav .has-child > a");
	...
	function site_search2() {
		...
		jQuery("#search-two, #search-two button.close").on(
			"click keyup",
			function (event) {
	...
	function search_13() {
		...
		jQuery("#search13, #search13 button.close").on(
			"click keyup",
			function (event) {
				if (event.target === this || event.target.className === "close") {
					jQuery(this).removeClass("open");
